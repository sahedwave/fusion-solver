...
def _scattering_source(
    ...
    """
    # P0 term: (nx,ny,nz,G) × (G,G) → (nx,ny,nz,G)
    q_p0 = np.tensordot(phi, mat.sigma_s0, axes=([3], [0]))
    Q_s = q_p0[:, :, :, np.newaxis, :]                        # broadcast over m

    # P1 term
    omega_dot_J = np.einsum('mc,ijkgc->ijkmg', directions, J)
    q_p1 = 3.0 * np.tensordot(omega_dot_J, mat.sigma_s1, axes=([4], [0]))
...
def _scattering_source_direction_group(
    ...
    q_p0 = np.tensordot(phi, mat.sigma_s0[:, group], axes=([3], [0]))
    omega_dot_J = np.einsum("c,ijkgc->ijkg", direction, J)
    q_p1 = 3.0 * np.tensordot(omega_dot_J, mat.sigma_s1[:, group], axes=([3], [0]))
    return q_p0 + q_p1
...
class DSAPreconditioner:
    ...
    *not* the removal cross section.  This is the standard DSA formulation.
    ...
    def apply(self, residual_phi: np.ndarray) -> np.ndarray:
        ...
        return sol.reshape(mesh.nx, mesh.ny, mesh.nz, G)
    ...
    def _assemble(
        ...
        """
        Assemble the sparse G×(nx·ny·nz) diffusion matrix.

        Uses Σ_a = sigma_t - sigma_s0.sum(axis=1) (P0 absorption only).
        The removal cross section is NOT used here — this is deliberate
        and consistent with standard DSA derivations.
        """
        nx, ny, nz, G = mesh.nx, mesh.ny, mesh.nz, mat.G
        dx, dy, dz    = mesh.dx, mesh.dy, mesh.dz
        D             = mat.D          # (G,)  diffusion coefficients
        sigma_a       = mat.sigma_a    # (G,)  P0 absorption
            ...
            for j in range(ny):
                for k in range(nz):
                    for g in range(G):
                        ...
                        cx = Dg / dx**2
                        if i > 0:
                            A[row, idx(i-1, j, k, g)] -= cx
                            diag += cx
                        elif bc.xmin:                       # reflective: zero flux gradient
                            diag += 0.0                     # no off-diagonal; natural BC
                        else:                               # vacuum: extrapolation
                            diag += 2.0 * Dg / dx          # half-cell extrapolation
